<?php
    ...
    if(!$result || mysqli_num_rows($result) <= 0){
	    ...
	    VALUES ("'.$_SESSION['id'].'","'.$item.'","1","'.$price.'","cart")';
	    mysqli_query($conn,$qry);
    }
    else{
        $qry2 = "UPDATE orders SET ammountOrdered = ammountOrdered + 1, price = ammountOrdered *" . $price . " WHERE productID =" . $item . " AND customerID=" . $_SESSION['id'];
